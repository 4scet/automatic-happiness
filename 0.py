...
def egb_spider(show=True):

    sleeptime = 1  # edit sleeptime, when you need to change delay(time) to load webpage
    ...
    for item in event:
        ...
        esport = esport_block.find('img')
        #('img', title=True)  #('img'['alt'])  # name of the e-sport
